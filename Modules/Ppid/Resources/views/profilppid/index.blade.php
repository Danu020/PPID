    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <a href="{{ route('profilppid.create') }}" class="btn btn-primary">+ Tambah</a>
            </div>
            ...
            <div class="table-responsive">
                <table id="example" class="table table-bordered table-striped align-middle">
                    ...
                    <tbody>
                        @forelse ($profilppid as $index => $item)
                        <tr>
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>{{ $item->judul }}</td>
                            <td>{{ \Illuminate\Support\Str::limit($item->deskripsi, 100) }}</td>
                            <td class="text-center">
                                <a href="{{ route('profilppid.edit', $item->id) }}" class="btn btn-warning btn-sm mb-1">Edit</a>
                                <form action="{{ route('profilppid.destroy', $item->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
